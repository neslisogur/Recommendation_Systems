...
df_ = pd.read_csv("recommendation_systems/ArmutARL/armut_data.csv")
df = df_.copy()
df.head()
df.info()
...
df.head()
df['Hizmet'] = df['Hizmet'].astype(int)
...
df_new = df.groupby(['SepetID', 'Hizmet'])['Hizmet'].sum().unstack().fillna(0).applymap(lambda x: 1 if x > 0 else 0)
